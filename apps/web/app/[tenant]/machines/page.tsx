...
import { Plus, Search, Pencil, Trash2, X, Cog } from 'lucide-react';
...
const FORM_VIDE = { code: '', nom: '', type: '', capacite: '', unite: 'kg/h' };
...
export default function MachinesPage() {
  ...
  const creerMutation = useMutation({
    mutationFn: (d: typeof formData) => api.post('/machines', { ...d, capacite: d.capacite ? +d.capacite : undefined }),
  ...
  const modifierMutation = useMutation({
    mutationFn: (d: typeof formData) => api.put(`/machines/${selected!.id}`, { ...d, capacite: d.capacite ? +d.capacite : undefined }),
  ...
  const ouvrirEdition = (m: Machine) => {
    setSelected(m);
    setFormData({ code: m.code, nom: m.nom, type: m.type, capacite: m.capacite?.toString() ?? '', unite: m.unite ?? 'kg/h' });
    setModal('edit');
  };
            ...
            <div key={m.id} className="bg-white rounded-xl border shadow-sm p-4">
              <div className="flex items-start justify-between mb-3">
                ...
                <div className="flex gap-1">
                  <button type="button" onClick={() => ouvrirEdition(m)}
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg">
            ...
            <div className="p-6 grid grid-cols-2 gap-4">
              {[
                ...
                <div key={key} className={key === 'nom' ? 'col-span-2' : ''}>
